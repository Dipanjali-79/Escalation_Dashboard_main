...
import videoconLogo from './assets/videocon.png';
import saloraLogo from './assets/salora.png';
import ledvanceLogo from './assets/ledvance.jpg';
...
const App = () => {
      ...
      try {
        ...
        if (parsed && typeof parsed === 'object') {
          ...
          if (parsed.role !== 'ADMIN') {
            if (parsed.roleType === 'BRAND') {
              setFilters(prev => ({ ...prev, brand: parsed.role, branch: '' }));
              setFormData(prev => ({ ...prev, brand: parsed.role, branch: '' }));
            } else {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${API_URL}?limit=100000`);
  ...
  // Optimized single-loop KPI selector
  const kpiStats = useMemo(() => {
    let openCount = 0;
    let closedCount = 0;
    let cancelledCount = 0;
    let agingOver5Count = 0;
    ...
    if (Array.isArray(filteredData)) {
      filteredData.forEach(d => {
        ...
        if (s !== 'closed' && s !== 'cancelled' && Number(d.aging || 0) > 5) {
          agingOver5Count++;
        }
      });
    }
  ...
  const handleLogin = (e) => {
    ...
    if (loginMode === 'ADMIN') {
      ...
      if (!["SALORA", "AERO NERO", "LEDVANCE", "VIDEOCON"].includes(uUpper)) {
        setLoginError('Only AERO NERO, LEDVANCE, SALORA, or VIDEOCON are permitted for Brand Login.');
        return;
      }
  ...
  const reportData = useMemo(() => {
    ...
    const relevant = user?.role === "ADMIN" ? data : data.filter(d => {
      const uRole = String(user?.role || "").toLowerCase();
      if (user?.roleType === 'BRAND') {
        const dBrand = String(d.brand || "").toLowerCase();
        return dBrand === uRole;
      }
    ...
    const brandFiltered = deferredFilters.brand ? filtered.filter(d => {
      return String(d.brand || "").trim().toLowerCase() === String(deferredFilters.brand).trim().toLowerCase();
    }) : filtered;
    ...
    <div className="app-container">
      ...
      <aside className={`sidebar ${isSidebarCollapsed ? 'collapsed' : ''}`}>
        <div className="brand">
          <img 
            src={
              user?.role === 'VIDEOCON' ? videoconLogo : 
              user?.role === 'SALORA' ? saloraLogo : 
              user?.role === 'LEDVANCE' ? ledvanceLogo : 
              logo
            } 
            className={`brand-logo ${['VIDEOCON', 'SALORA', 'LEDVANCE'].includes(user?.role) ? 'custom-brand' : ''}`} 
            alt="Logo" 
          />
          {!isSidebarCollapsed && !['VIDEOCON', 'SALORA', 'LEDVANCE'].includes(user?.role) && (
                ...
                <div className="metrics-grid">
                  <div className="stat-card">
                    <div className="stat-info">
                      <h4>Total Cases</h4>
                      <div className="value">{kpiStats.total}</div>
                    </div>
                  ...
                  <div className="stat-card">
                    <div className="stat-info">
                      <h4>Total Open</h4>
                      <div className="value">{kpiStats.open}</div>
                    </div>
                  ...
                  <div className="stat-card">
                    <div className="stat-info">
                      <h4>Closed</h4>
                      <div className="value">{kpiStats.closed}</div>
                    </div>
                  ...
                  <div className="stat-card">
                    <div className="stat-info">
                      <h4>Cancelled</h4>
                      <div className="value">{kpiStats.cancelled}</div>
                    </div>
                  ...
                  {filteredData.length > 0 && (
                    <div className="table-pagination">
                      <div className="pagination-info">
                        Showing <b>{Math.min(filteredData.length, (currentPage - 1) * rowsPerPage + 1)}</b> to <b>{Math.min(filteredData.length, currentPage * rowsPerPage)}</b> of <b>{filteredData.length}</b> cases
                      </div>
                      <div className="pagination-controls">
                        ...
                        <div className="flex gap-1" style={{ display: 'flex', alignItems: 'center' }}>
                          ...
                          {getPaginationRange(currentPage, Math.ceil(filteredData.length / rowsPerPage)).map((pg, idx) => (
                            pg === '...' ? (
                              <span key={`dots-${idx}`} className="pagination-dots" style={{ padding: '0 0.25rem', color: 'var(--text-muted)', fontSize: '0.875rem' }}>...</span>
                            ) : (
                              <button 
                                key={`pg-${pg}`} 
                          ...
                          <button 
                            ...
                            onClick={() => setCurrentPage(prev => Math.min(Math.ceil(filteredData.length / rowsPerPage), prev + 1))}
                            title="Next Page"
                          >
                            ›
                          </button>
